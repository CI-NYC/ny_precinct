...
### ALLOWING FOR 43, 47, 49 AS CONTROLS

#### Using ridge to allow for extraploation (lambda selected through a visual analysis)
```{r}
temp <- merged_dat |>
  filter(year >= 2006) |>
  select(PCT, GROUP, year, y, treated) |>
  mutate(treated = ifelse(year > 2014 & treated == 1, 1, 0)) |>
  rename("shootings" = "y",
         "case" = "treated") |>
  mutate(PCT = as.character(GROUP)) |>
  select(-GROUP) |>
  as.data.frame()
...
### NOT ALLOWING FOR 43, 47, 49 AS CONTROLS
